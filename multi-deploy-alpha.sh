...
do
	...
	# Update system and Install Certbot which will generate SSL certificates for the server
	echo "${GREEN}Performing system update.${NC}"
	ssh root@${HOSTNAME} "sudo apt-get install && apt-get update > /dev/null"
	ssh root@${HOSTNAME} "sudo apt-get install software-properties-common > /dev/null"
	ssh root@${HOSTNAME} "sudo add-apt-repository universe > /dev/null"
	ssh root@${HOSTNAME} "sudo add-apt-repository ppa:certbot/certbot > /dev/null"
	ssh root@${HOSTNAME} "sudo apt-get update > /dev/null"
	ssh root@${HOSTNAME} "sudo apt-get install certbot python-certbot-nginx > /dev/null"
	ssh root@${HOSTNAME} "sudo certbot --nginx > /dev/null"
	...
	#Edit postfix main.cf before reboot
